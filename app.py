...
# ────────────────────────────────────────────────────────────
# 0. ARABIC TEXT SETUP FOR MATPLOTLIB
# ────────────────────────────────────────────────────────────
...
def find_active_wolfe(df, max_bars_since_p5=8, pivot_orders=None):
...
def plot_wolfe_chart(ticker, df, result, tf_label):
    ...
    ax.plot(
        z_last, target, marker='D', ms=14, color=C_T,
    ...
    # Title with Arabic
    emoji = '\U0001F4C8' if is_bull else '\U0001F4C9'
    title_text = f'{emoji}   {ticker}  -  {ar_name_shaped}   |   {direction_shaped}   |   {tf_shaped}'
    ax.set_title(title_text, fontsize=16, fontweight='bold', pad=16, color='#212121')
    ax.set_ylabel('')
